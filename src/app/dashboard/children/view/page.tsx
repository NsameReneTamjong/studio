...
const MOCK_GRADES = [
  { subject: "Physics", seq1: 14.5, seq2: 16.0, teacher: "Dr. Tesla", status: "Passed", coeff: 4, group: "Sciences" },
  { subject: "Mathematics", seq1: 18.0, seq2: 17.5, teacher: "Prof. Smith", status: "Passed", coeff: 5, group: "Sciences" },
  { subject: "English Literature", seq1: 12.0, seq2: 13.0, teacher: "Ms. Bennet", status: "Passed", coeff: 3, group: "Arts" },
  { subject: "Chemistry", seq1: 10.5, seq2: 11.5, teacher: "Dr. White", status: "Passed", coeff: 4, group: "Sciences" },
];
...
export default function StudentDetailsPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" className="gap-2" onClick={() => setIsMessageModalOpen(true)}>
            <Mail className="w-4 h-4" /> {language === 'en' ? 'Message Instructor' : 'Contacter Enseignant'}
          </Button>
          ...
          <Card className="border-none shadow-xl overflow-hidden rounded-3xl">
            <CardHeader className="bg-primary p-8 text-white">
              <div className="flex flex-row items-center justify-between">
                ...
                <Button onClick={() => setPreviewDoc({ type: 'report' })} className="gap-2 bg-secondary text-primary shadow-lg border-none hover:bg-secondary/90">
                  ...
                  {MOCK_GRADES.map((grade: any, idx: number) => {
                    ...
                    return (
                      <TableRow key={idx} className="hover:bg-accent/5 border-b border-accent/10">
                        <TableCell className="font-bold pl-8 py-4 text-primary">{grade.subject}</TableCell>
            ...
            <CardFooter className="bg-primary/5 p-6 flex justify-between items-center border-t">
               <p className="text-xs font-bold text-primary italic">{language === 'en' ? "Calculated based on verified school coefficients." : "Calculé sur la base des coefficients scolaires vérifiés."}</p>
      ...
      {/* Report Card Modal Logic remains consistent with the updated one above */}
    </div>
  );
}
