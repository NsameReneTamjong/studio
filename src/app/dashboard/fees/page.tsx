...
const MOCK_STUDENT_LEDGER = [
  { id: "GBHS26S001", name: "Alice Thompson", avatar: "https://picsum.photos/seed/s1/100/100", paid: 125000, left: 25000, status: "partial", isLicensePaid: true, class: "2nde / Form 5" },
  { id: "GBHS26S002", name: "Bob Richards", avatar: "https://picsum.photos/seed/s2/100/100", paid: 150000, left: 0, status: "cleared", isLicensePaid: true, class: "Terminale / Upper Sixth" },
  { id: "GBHS26S003", name: "Charlie Davis", avatar: "https://picsum.photos/seed/s3/100/100", paid: 45000, left: 105000, status: "partial", isLicensePaid: false, class: "1ère / Lower Sixth" },
  { id: "GBHS26S004", name: "Diana Prince", avatar: "https://picsum.photos/seed/s4/100/100", paid: 150000, left: 0, status: "cleared", isLicensePaid: true, class: "2nde / Form 5" },
];

const RECENT_TRANSACTIONS = [
  { id: "PAY-001", student: "Alice Thompson", type: "Tuition Fee", amount: "50,000", method: "Cash", date: "Today, 10:30 AM" },
  { id: "PAY-002", student: "Bob Richards", type: "Uniform Package", amount: "25,000", method: "MoMo", date: "Today, 09:15 AM" },
];
  ...
  const handleProcessPayment = () => {
    ...
    setTimeout(() => {
      const receipt = {
        ...
        amount: parseInt(paymentForm.amount).toLocaleString(),
      ...
      toast({ title: "Payment Recorded", description: "Official receipt generated." });
    }, 1500);
  };
    ...
    <div className="space-y-8 pb-20">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage intake, record payments, and audit institutional revenue.</p>
          </div>
        </div>
      </div>
      ...
      <Tabs defaultValue="pay" className="w-full">
        <TabsList className="grid grid-cols-3 w-full md:w-[600px] mb-8 bg-white shadow-sm border h-auto p-1 rounded-2xl">
          <TabsTrigger value="pay" className="gap-2 py-3 rounded-xl transition-all">
            <Wallet className="w-4 h-4" /> Collection Desk
          </TabsTrigger>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredStudents.map((s) => (
                    <TableRow key={s.id} className="group hover:bg-accent/5 transition-colors border-b">
                      ...
                      <TableCell className="text-center">
                        {s.isLicensePaid ? (
                          <Badge className="bg-green-100 text-green-700 border-none text-[9px] font-black uppercase">Active Node</Badge>
                        ) : (
                          <Badge variant="destructive" className="bg-red-100 text-red-700 border-none text-[9px] font-black uppercase">Unpaid License</Badge>
                        )}
                      </TableCell>
                      <TableCell className="text-right pr-8">
                        <Button 
                          size="sm" 
                          className="rounded-xl h-9 px-6 font-black uppercase tracking-widest text-[10px] shadow-lg"
                          disabled={!s.isLicensePaid}
                          onClick={() => setSelectedStudentForPayment(s)}
                        >
                          <Wallet className="w-3.5 h-3.5 mr-2" /> Pay Fee
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <TabsContent value="ledger" className="mt-0">
          <Card className="border-none shadow-xl overflow-hidden rounded-3xl">
            <CardHeader className="bg-white border-b p-6">
              ...
              <CardDescription>Verified chronological record of physical and digital intake.</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
      ...
      <Dialog open={!!selectedStudentForPayment} onOpenChange={() => setSelectedStudentForPayment(null)}>
        <DialogContent className="sm:max-w-md rounded-[2.5rem] p-0 overflow-hidden border-none shadow-2xl">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input 
                    type="number" 
                    placeholder="0" 
                    className="h-14 bg-accent/30 border-none rounded-2xl font-black text-xl text-primary pl-6"
          ...
          <DialogFooter className="bg-accent/20 p-6 border-t border-accent">
            <Button 
              className="w-full h-14 rounded-2xl shadow-xl font-black uppercase tracking-widest text-xs gap-3 bg-primary text-white" 
          ...
          <DialogHeader className="bg-primary p-8 text-white no-print">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <DialogTitle className="text-2xl font-black">Official Receipt</DialogTitle>
                  <DialogDescription className="text-white/60">Institutional transaction recorded.</DialogDescription>
                </div>
              </div>
               ...
               <div className="flex justify-between items-start border-b-2 border-black pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[8px] font-bold uppercase opacity-60">Financial Services Registry</p>
                    </div>
                  </div>
               ...
               <div className="grid grid-cols-2 gap-8 py-4">
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-[9px] font-mono font-bold text-primary">{issuedReceipt?.studentId} • {issuedReceipt?.class}</p>
                    </div>
                    <div>
                      <p className="text-[8px] font-black uppercase text-muted-foreground tracking-widest">Charge Category</p>
                      <p className="font-black text-sm uppercase">{issuedReceipt?.feeType}</p>
                    </div>
                  </div>
                  <div className="space-y-4 text-right">
                    <div>
                      <p className="text-[8px] font-black uppercase text-muted-foreground tracking-widest">Transaction Date</p>
                      <p className="font-bold text-sm">{issuedReceipt?.date}</p>
                    </div>
                    <div className="p-3 bg-primary text-white rounded-xl shadow-inner">
                      ...
                      <p className="font-black text-lg text-secondary underline underline-offset-4 decoration-double">{issuedReceipt?.amount} XAF</p>
                    </div>
                  </div>
               </div>
